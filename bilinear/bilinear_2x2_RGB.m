function out = bilinear_2x2_RGB(img, STEP = 0.1)
    ...
    %R_matrix = zeros(length(R), 3 * length(R));
    %G_matrix = zeros(length(R), length(R));
    %B_matrix = zeros(p, q);
    
    R_matrix = bilinear_2x2(R, STEP);
    G_matrix = bilinear_2x2(G, STEP);
    B_matrix = bilinear_2x2(B, STEP);
